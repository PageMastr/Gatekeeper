...
##
## PROJECT-SPECIFIC PRESETS DO NOT BELONG HERE. Put them in
## `res://scripts/project_presets.gd` - see `all_presets()` below. A preset
## naming a palette key that only one game defines is broken in every other
## game: observed, two presets referencing `sky_nebula_far` leaked into three
## games that have no such swatch, and a later reinstall then deleted them from
## the game that wrote them, because nothing owned them.
...
## -1 means "ask the project". Resolved in _ready() from GDProject.BUDGET, so a
## game whose budget allows one shadow caster enforces one at runtime instead of
## enforcing the shared default of four and then failing its own gate at one.
@export var shadow_budget: int = -1
...
func _ready() -> void:
	...
	if shadow_budget < 0:
		shadow_budget = 4
		if ResourceLoader.exists("res://scripts/gd_project.gd"):
			var proj = load("res://scripts/gd_project.gd")
			if proj != null and "BUDGET" in proj:
				shadow_budget = int(proj.BUDGET.get("max_shadow_casting_lights", 4))
	_refresh()
...
## THE EXTENSION POINT for game-specific presets.
##
## A game adds its own looks in `res://scripts/project_presets.gd`:
##
##     const PRESETS := { "cabin_interior": { ...same shape as below... } }
##
## They are merged on top of the shipped library at load, so `apply_preset()`
## and the F9 panel find them like any other. They live in the project, under
## the project's version control, and survive every harness reinstall.
##
## This exists because the alternative was observed and it was bad: a game put
## two of its presets in this file, which is installed once per machine, and
## they propagated into three unrelated projects - two of which had no palette
## swatch for them to resolve against. Then a reinstall deleted them from the
## game that wrote them, because nothing owned them.
const _PROJECT_PRESETS_PATH := "res://scripts/project_presets.gd"
static var _all_presets: Dictionary = {}
...
static func all_presets() -> Dictionary:
	...
	_all_presets = PRESETS.duplicate(true)
	if ResourceLoader.exists(_PROJECT_PRESETS_PATH):
		var scr = load(_PROJECT_PRESETS_PATH)
		if scr != null and "PRESETS" in scr:
			for k in scr.PRESETS:
				_all_presets[k] = scr.PRESETS[k]
	return _all_presets
...
func _refresh() -> void:
	var lib := all_presets()
	if not lib.has(preset):
		return
	var p: Dictionary = lib[preset]
...
func _process(delta: float) -> void:
	...
	var e: float = float(all_presets()[preset]["ambient_energy"])
	if world and world.environment:
